...
export const INVGPIO_VALIDATOR = {
  validator(rule: InternalRuleItem, value: number, callback: (error?: string) => void) {
...
export const createSettingsValidator = (settings: Settings) =>
  new Schema({
    ...(settings.board_profile === 'CUSTOM' && {
      ...
      tx_gpio: [{ required: true, message: 'Tx GPIO is required' }, INVGPIO_VALIDATOR],
      rx_gpio: [{ required: true, message: 'Rx GPIO is required' }, INVGPIO_VALIDATOR]
    }),
